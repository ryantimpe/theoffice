...
gt_merge_stack_tooltip<- function(gt_object, col1, col2, 
  ...
  gt_object %>%
    text_transform(
      ...
      fn = function(x) {
        ...
        glue::glue(
          "<div class='gtExtras_tooltip'>{x}
            <span class='gtExtras_tooltipcontent'>{data_in}</span>
          </div>"
    ...
    opt_css(
      "
        ...
        .gtExtras_tooltip:hover span.gtExtras_tooltip{
            display:block;
        }
      "
    ) %>% 
